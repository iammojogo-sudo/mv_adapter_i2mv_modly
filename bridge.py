...
def normalize_view_to_frame(img, resolution, fill=0.9, bg=128):
    """Recenter and rescale the subject in a generated view so every view
    shares the same framing: subject bbox scaled to `fill` of the frame,
    centered, on a uniform gray background. The diffusion model drifts object
    scale/position between views; consistent framing is what lets the mesh
    generator triangulate cleanly and the texture bake blend views without
    seams or warping strain. Returns the input unchanged when the subject
    cannot be reliably segmented."""
    try:
        arr = np.array(img.convert("RGB"))
        fg = _subject_mask(arr)
        if not (0.02 < fg.mean() < 0.98):
            return img
        ys, xs = np.where(fg)
        crop = arr[ys.min():ys.max() + 1, xs.min():xs.max() + 1]
        ch, cw = crop.shape[:2]
        target = int(resolution * fill)
        sc = target / max(ch, cw, 1)
        nh, nw = max(1, int(ch * sc)), max(1, int(cw * sc))
        crop = np.array(Image.fromarray(crop).resize((nw, nh), Image.LANCZOS))
        out = np.full((resolution, resolution, 3), bg, dtype=np.uint8)
        sh, sw = (resolution - nh) // 2, (resolution - nw) // 2
        out[sh:sh + nh, sw:sw + nw] = crop
...
def run_mv_adapter(mesh_path, ref_image_path, output_dir, device="cuda",
                   ...
                   use_reference_front=True, normalize_views=True):
    ...
    # Post-process the sweep. The model REGENERATES the front view, so it never
    # exactly matches the input photo (small azimuth/scale drift that compounds
    # downstream). Use the real reference as front — pixel-exact for mesh
    # conditioning and for the texture hybrid warp, which then only has to fit
    # the genuine side/back views. Normalizing framing on the rest keeps every
    # view centered at the same subject scale.
    if use_reference_front:
        images[0] = ref_img.copy()
    if normalize_views:
        images = [normalize_view_to_frame(im, resolution) for im in images]
...
if __name__ == "__main__":
    ...
    use_reference_front = bool(args.get("use_reference_front", True))
    normalize_views = bool(args.get("normalize_views", True))
